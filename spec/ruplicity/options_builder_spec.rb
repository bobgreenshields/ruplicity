...
describe OptionsBuilder do
	describe "#normalise_keys" do
		let (:optb) { OptionsBuilder.new }
		before :each do
			@args = {"name" => "base", FORCE: nil, "Dry-Run" => nil }
		end

		it "should change strings to symbols" do
			optb.normalise_keys(@args).keys[0].should eql(:name)
		end

		it "should downcase symbol names" do
			optb.normalise_keys(@args).keys[1].should eql(:force)
		end

		it "should downcase strings and change - to _" do
			optb.normalise_keys(@args).keys[2].should eql(:dry_run)
		end

		it "should not change any of the values" do
			optb.normalise_keys(@args)[:name].should eql("base")
			optb.normalise_keys(@args)[:force].should eql(nil)
			optb.normalise_keys(@args)[:dry_run].should eql(nil)
		end
	end
	...
	describe "#key_from_opt" do
		...
		it "should change any option into a key symbol" do
			optb.key_from_opt("--dry-run").should eql(:dry_run)
		end
	end
	...
	describe "#argless_options_from_keys" do
		...
		it "should prepend an option for a key if not present" do
			...
			optb.argless_options_from_keys(args, opts).should eql(expected)
		end

		it "should ignore a key if the option is already present" do
			optb.argless_options_from_keys(@args, @opts)[2].should eql("--dry-run")
			optb.argless_options_from_keys(@args, @opts)[0].should eql("--force")
		end
	end
	...
	describe "#options_with_args_from_keys" do
		...
		it "should return an option for a key if not present" do
			expected = ["--name thisname", "--encrypt-key ABB12345", 
							 ...
							 "--verbosity 8"] + @opts
			optb.options_with_args_from_keys(@args, @opts).should eql(expected)
		end
